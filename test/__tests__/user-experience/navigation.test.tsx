...
function TestComponent({ workspaceSlug = 'test-workspace' }: { workspaceSlug?: string }) {
  ...
  describe('Router Navigation', () => {
    ...
    it('prefetches routes on hover', async () => {
      render(
        <TestWrapper>
          <button 
            onMouseEnter={() => mockRouter.prefetch('/test-workspace/issues/1')}
          >
            Issue Link
          </button>
        </TestWrapper>
  ...
  describe('Deep Linking', () => {
    it('supports direct navigation to issue details', async () => {
      ...
      render(
        <TestWrapper>
          <button 
            onClick={() => mockRouter.push(`/${workspaceSlug}/issues/${issueId}`)}
          >
            Go to Issue
          </button>
        </TestWrapper>
    ...
    it('preserves query parameters during navigation', async () => {
      render(
        <TestWrapper>
          <button 
            onClick={() => mockRouter.push('/test-workspace?filter=todo&priority=high')}
          >
            Filtered View
          </button>
        </TestWrapper>
