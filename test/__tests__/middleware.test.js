...
vi.mock('next/server', () => ({
    NextResponse: {
        next: vi.fn((init) => (Object.assign({ cookies: {
                set: vi.fn(),
            } }, init))),
        redirect: vi.fn((url) => ({
            type: 'redirect',
    ...
    describe('authenticated user - onboarding flow', () => {
        ...
        describe('no profile, no workspace', () => {
            ...
            it('redirects from /CreateWorkspace to /CreateUser', async () => {
                mockRequest.nextUrl.pathname = '/CreateWorkspace';
                await middleware(mockRequest);
                expect(NextResponse.redirect).toHaveBeenCalledWith(new URL('/CreateUser', mockRequest.url));
            });
            ...
            it('redirects from home to /workspace', async () => {
                mockRequest.nextUrl.pathname = '/';
                await middleware(mockRequest);
                expect(NextResponse.redirect).toHaveBeenCalledWith(new URL('/workspace', mockRequest.url));
            });
        });
        describe('has profile, no workspace', () => {
            ...
            it('redirects from /CreateUser to /CreateWorkspace', async () => {
                mockRequest.nextUrl.pathname = '/CreateUser';
                await middleware(mockRequest);
                expect(NextResponse.redirect).toHaveBeenCalledWith(new URL('/CreateWorkspace', mockRequest.url));
            });
        ...
        describe('has profile and workspace', () => {
            ...
            it('redirects from /CreateUser to /success', async () => {
                mockRequest.nextUrl.pathname = '/CreateUser';
                await middleware(mockRequest);
                expect(NextResponse.redirect).toHaveBeenCalledWith(new URL('/success', mockRequest.url));
            });
