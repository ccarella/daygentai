...
describe('Database Transaction Handling', () => {
    ...
    describe('Atomic Operations', () => {
        it('ensures single operations are atomic', async () => {
            ...
            const selectMock = vi.fn()
                ...
                .mockResolvedValueOnce({
                data: Object.assign(Object.assign({}, newIssue), { id: 'new-id' }),
                error: null
            });
    ...
    describe('Batch Operations', () => {
        it('handles batch inserts atomically', async () => {
            ...
            batchIssues[2] = Object.assign(Object.assign({}, batchIssues[2]), { workspace_id: 'invalid-workspace' });
            const insertMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: null,
                error: {
        ...
        it('handles batch updates atomically', async () => {
            ...
            const bulkUpdate = { status: 'done' };
            const updateMock = vi.fn().mockReturnThis();
            const inMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: issueIds.map(id => (Object.assign({ id }, bulkUpdate))),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
            ...
            expect(data.every((issue) => issue.status === 'done')).toBe(true);
    ...
    describe('Concurrent Access Handling', () => {
        it('handles optimistic locking scenarios', async () => {
            ...
            const user1Update = { status: 'in_progress' };
            const user2Update = { status: 'in_review' };
            // User 1's update
            const updateMock1 = vi.fn().mockReturnThis();
            const eqMock1 = vi.fn().mockReturnThis();
            const selectMock1 = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign(Object.assign({}, issue), user1Update), { updated_at: '2024-01-01T00:00:01Z' }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock1,
            ...
            const selectMock2 = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign(Object.assign({}, issue), user2Update), { updated_at: '2024-01-01T00:00:02Z' }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock2,
        ...
        it('handles race conditions in unique constraint scenarios', async () => {
            ...
            const selectMock1 = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, workspace1), { id: 'workspace-1-id' }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                insert: insertMock1,
        ...
        it('maintains data consistency during complex operations', async () => {
            ...
            const createWorkspaceWithIssues = async () => {
                ...
                const selectWorkspaceMock = vi.fn().mockResolvedValue({
                    data: Object.assign(Object.assign({}, workspace), { id: 'new-workspace-id' }),
                    error: null
                });
                mockSupabaseClient.from.mockReturnValue({
                ...
                const selectIssuesMock = vi.fn().mockResolvedValue({
                    data: issues.map((issue, i) => (Object.assign(Object.assign({}, issue), { id: `issue-${i}` }))),
                    error: null
                });
                mockSupabaseClient.from.mockReturnValue({
        ...
        it('allows retry after transient failures', async () => {
            ...
            const selectMock = vi.fn().mockImplementation(() => {
                ...
                if (attempts === 1) {
                    ...
                    return Promise.resolve({
                        data: Object.assign(Object.assign({}, issue), { id: 'new-issue-id' }),
                        error: null
                    });
                }
            });
