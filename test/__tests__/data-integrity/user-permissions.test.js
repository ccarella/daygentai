...
describe('User Permissions', () => {
    ...
    describe('User Profile Permissions', () => {
        ...
        it('allows users to update their own profile', async () => {
            ...
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, authenticatedUser), updates),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
    ...
    describe('Issue Assignee Permissions', () => {
        it('allows workspace owners to assign issues to any user', async () => {
            ...
            const eqMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, issue), { assignee_id: anotherUser.id }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
        ...
        it('allows self-assignment of issues', async () => {
            ...
            const eqMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, issue), { assignee_id: authenticatedUser.id }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
        ...
        it('allows unassigning issues', async () => {
            ...
            const eqMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, issue), { assignee_id: null }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
    ...
    describe('Data Visibility Scope', () => {
        it('users can only see data in their own workspaces', async () => {
            ...
            expect(data.every((issue) => issue.workspace_id === userWorkspace.id)).toBe(true);
            expect(error).toBeNull();
        });
    ...
    describe('Permission Inheritance', () => {
        it('workspace ownership grants full control over workspace issues', async () => {
            ...
            const eqMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, issue), { status: 'done' }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
