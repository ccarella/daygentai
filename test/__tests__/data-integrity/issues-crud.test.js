...
describe('Issue CRUD Operations', () => {
    ...
    describe('Create Operations', () => {
        it('creates a new issue with all required fields', async () => {
            ...
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, newIssue), { id: mockIssue.id, created_at: new Date().toISOString() }),
                error: null
            });
        ...
        it('accepts all valid issue types', async () => {
            for (const type of issueTypes) {
                ...
                const insertMock = vi.fn().mockReturnThis();
                const selectMock = vi.fn().mockResolvedValue({
                    data: Object.assign(Object.assign({}, issue), { id: mockIssue.id }),
                    error: null
                });
                mockSupabaseClient.from.mockReturnValue({
                    insert: insertMock,
        ...
        it('sets default values for optional fields', async () => {
            ...
            const insertMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, minimalIssue), { id: mockIssue.id, type: 'task', priority: 'medium', description: null, assignee_id: null, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                insert: insertMock,
    ...
    describe('Read Operations', () => {
        ...
        it('filters issues by status', async () => {
            ...
            // Chain eq calls
            let callCount = 0;
            eqMock.mockImplementation((field, value) => {
                callCount++;
                if (callCount === 2) {
            ...
            expect(data.every((issue) => issue.status === 'todo')).toBe(true);
            expect(error).toBeNull();
        });
    ...
    describe('Update Operations', () => {
        it('updates issue status', async () => {
            ...
            const eqMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, mockIssue), { status: newStatus }),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
        ...
        it('updates multiple fields simultaneously', async () => {
            ...
            const eqMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn().mockResolvedValue({
                data: Object.assign(Object.assign({}, mockIssue), updates),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
        ...
        it('handles concurrent updates gracefully', async () => {
            ...
            const update2 = { status: 'in_review' };
            const updateMock = vi.fn().mockReturnThis();
            const eqMock = vi.fn().mockReturnThis();
            const selectMock = vi.fn()
                .mockResolvedValueOnce({
                data: Object.assign(Object.assign({}, mockIssue), update1),
                error: null
            })
                .mockResolvedValueOnce({
                data: Object.assign(Object.assign({}, mockIssue), update2),
                error: null
            });
            mockSupabaseClient.from.mockReturnValue({
                update: updateMock,
