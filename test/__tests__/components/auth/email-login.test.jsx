...
describe('EmailLogin', () => {
    ...
    describe('form submission', () => {
        ...
        it('redirects to check email page on success', async () => {
            ...
            await user.click(submitButton);
            await waitFor(() => {
                expect(mockRouter.push).toHaveBeenCalledWith('/checkemail?email=test%40example.com');
            });
        });
        it('shows loading state during submission', async () => {
            // Mock a slow response
            mockSupabase.auth.signInWithOtp.mockImplementationOnce(() => new Promise(resolve => setTimeout(() => resolve({ data: null, error: null }), 100)));
            render(<EmailLogin />);
            const emailInput = screen.getByLabelText('Email address');
            await user.type(emailInput, 'test@example.com');
            const submitButton = screen.getByRole('button', { name: 'Send login link' });
    ...
    describe('error handling', () => {
        it('displays error message when OTP fails', async () => {
            ...
            await waitFor(() => {
                ...
                const classNames = (errorContainer === null || errorContainer === void 0 ? void 0 : errorContainer.className) || '';
        ...
        it('recovers from error state on new submission', async () => {
            ...
            await waitFor(() => {
                ...
                expect(mockRouter.push).toHaveBeenCalledWith('/checkemail?email=test2%40example.com');
            });
        });
    });
    describe('edge cases', () => {
        it('handles email with special characters', async () => {
            ...
            await waitFor(() => {
                ...
                expect(mockRouter.push).toHaveBeenCalledWith('/checkemail?email=test%2Btag%40example.com');
            });
        });
        it('prevents multiple simultaneous submissions', async () => {
            // Mock a slow response
            let resolvePromise;
            mockSupabase.auth.signInWithOtp.mockImplementationOnce(() => new Promise(resolve => { resolvePromise = resolve; }));
            render(<EmailLogin />);
            const emailInput = screen.getByLabelText('Email address');
            await user.type(emailInput, 'test@example.com');
            const submitButton = screen.getByRole('button', { name: 'Send login link' });
