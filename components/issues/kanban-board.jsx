...
export function KanbanBoard({ workspaceId, onIssueClick, statusFilter = 'exclude_done', priorityFilter = 'all', typeFilter = 'all' }) {
    ...
    const fetchIssues = useCallback(async (pageNum, append = false) => {
        if (loadingMoreRef.current && append)
            return;
    ...
    const handleDrop = async (e, newStatus) => {
        ...
        const { error } = await supabase
            ...
            .eq('id', issueId);
        if (!error) {
            setIssues(prev => prev.map(issue => issue.id === issueId ? Object.assign(Object.assign({}, issue), { status: newStatus }) : issue));
        }
    };
    ...
    return (<div className="h-full flex flex-col mt-3">
      {hasMore && (<div className="flex items-center justify-end mb-4 text-sm">
          <button onClick={loadMore} className="text-blue-600 hover:text-blue-800" disabled={loadingMoreRef.current}>
      ...
      <div className="flex gap-4 h-full overflow-x-auto pb-4 px-4">
        {columns.map(column => {
            ...
            return (<div key={column.id} className="flex-shrink-0 w-72" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, column.id)}>
              ...
              <div className="bg-gray-50 border border-t-0 rounded-b-lg min-h-[400px] max-h-[calc(100vh-300px)] overflow-y-auto p-2 space-y-2">
                {columnIssues.length === 0 ? (<p className="text-gray-400 text-sm text-center py-8">
                    No issues
                  </p>) : (columnIssues.map(issue => (<div key={issue.id} draggable onDragStart={(e) => handleDragStart(e, issue.id)} onClick={() => onIssueClick(issue.id)} className="bg-white p-3 rounded-lg border shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                      ...
                      {issue.description && (<p className="text-sm text-gray-600 line-clamp-2 mb-2">
                          {issue.description}
                        </p>)}
