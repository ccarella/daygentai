...
export function IssuesList({ workspaceId, workspaceSlug, onIssueClick, statusFilter = 'exclude_done', priorityFilter = 'all', typeFilter = 'all' }) {
    ...
    const preloadedIssuesRef = useRef(new Set());
      ...
      <div className="">
        ...
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-medium text-gray-600">
            {totalCount > 0 && issues.length < totalCount ? (<>Showing {issues.length} of {totalCount} {totalCount === 1 ? 'issue' : 'issues'}</>) : (<>{issues.length} {issues.length === 1 ? 'issue' : 'issues'}</>)}
          </h2>
        </div>
        
        {/* Issues List */}
        <div className="divide-y divide-gray-100">
          {issues.map((issue) => (<div key={issue.id} data-issue-id={issue.id} ref={(el) => {
                if (el && observerRef.current) {
                    observerRef.current.observe(el);
                }
            }} className="px-6 py-4 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => {
                if (onIssueClick) {
              ...
              <div className="flex-1 min-w-0">
                ...
                {issue.description && (<p className="mt-1 text-sm text-gray-600 line-clamp-2">
        ...
        {/* Load more button */}
        {hasMore && !initialLoading && (<div className="px-6 py-8 flex justify-center">
            <button onClick={handleLoadMore} disabled={loadingMore} className={`px-6 py-2 rounded-lg font-medium transition-all ${loadingMore
        ...
        {/* End of list message */}
        {!hasMore && issues.length > 0 && (<div className="px-6 py-8 text-center text-sm text-gray-500">
            All issues loaded
          </div>)}
      </div>
    </div>);
